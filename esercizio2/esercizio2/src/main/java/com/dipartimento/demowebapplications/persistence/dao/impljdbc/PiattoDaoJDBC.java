...
public class PiattoDaoJDBC implements PiattoDao {
    ...
    public Piatto findByPrimaryKey(String nome) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            if (resultSet.next()) {
                return new Piatto(resultSet.getString("nome"), resultSet.getString("ingredienti"), DBManager.getInstance().getRistoranteDao().findAllByPiattoName(resultSet.getString("nome")));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
